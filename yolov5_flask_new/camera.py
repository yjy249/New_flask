...
class Camera(BaseCamera):
    ...
    def frames():
        out, weights, imgsz = \
            ROOT / 'inference/output', ROOT / 'weights/best.pt', 640
        ...
        model = DetectMultiBackend(weights, device=device, dnn=False, data=ROOT / 'data/garbagess.yaml')
        ...
        if webcam:dataset = LoadStreams(Camera.video_source, img_size=imgsz)
        else:dataset = LoadImages(Camera.video_source, img_size=imgsz)
